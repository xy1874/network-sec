...
最后，本教程的核心结果来自测试攻击的结果。每次调用此测试函数都会在 MNIST 测试集上执行一个完整的测试步骤并报告最终精度。但是，请注意此函数还有一个 *epsilon* 参数输入，“测试”函数报告受攻击的模型的准确性来自  $\epsilon$ 的输入。更具体地说，对于测试集中的每个样本，该函数计算输入数据 ($data\_grad$) 的损失会产生扰动带有 “fgsm_attack”($perturbed\_data$) 的图像，然后检查以查看是否是对抗性的。除了测试模型的精度，该函数还保存并返回一些成功的对抗性示例，将在以后可视化。
...
例如，$\epsilon=0.05$ 时的精度比 $\epsilon=0$ 降低约 4% ，但 $\epsilon=0.2$ 时的精度却比 $\epsilon=0.15$ 降低约 25%  。另外，注意介于 $\epsilon=0.25$ 和 $\epsilon=0.3$ 模型的精度达到 10 类分类器的随机精度。
